...
function fchangeSchedule(){
    var schedule = $("input[name=fchangeScheduleContent]").val();
    var date = $("input[name=fchangeScheduleDate]").val();
    var start = $("input[name=fchangeScheduleStart]").val();
    var end = $("input[name=fchangeScheduleEnd]").val();
    var c = document.getElementById("fcloseChangeSchedule");
    var check = document.getElementsByName("fisChangeWatch");
    var isWatch = "";
    if (check[1].checked) {
        isWatch = check[1].value;
    }  
...
function changeSchedule(){
    ...
    var btn = document.getElementById("fchange");
...
function addOrder(){
    var date = $("input[name=arrangeDate]").val();
    var start = $("input[name=arrangeStart]").val();
    var end =$("input[name=arrangeEnd]").val();
    var schedule = $("input[name=arrangeContent]").val();
    var teacher = $("input[name=arrangeTeacher]").val();
    var c = document.getElementById("closeArrange");
    if (date.replace(/(^s*)|(s*$)/g, "").length == 0||schedule.replace(/(^s*)|(s*$)/g, "").length == 0|| teacher.replace(/(^s*)|(s*$)/g, "") == 0) {
        c.click();
        var onlyChoseAlert = simpleAlert({
            "content": "请输入预约信息",
            "buttons": {
...
function changeTeacherInf(){
    ...
    var position = $("input[name=teacherPositon]").val();
    var college = $("input[name=teacherCollege]").val();
    var money = $("input[name=teacherMoney]").val();
    var fsra = $("input[name=teacherFRSA]").val();
    if (money.replace(/(^s*)|(s*$)/g, "").length == 0  ||fsra.replace(/(^s*)|(s*$)/g, "").length == 0  ||name.replace(/(^s*)|(s*$)/g, "").length == 0  || college.replace(/(^s*)|(s*$)/g, "").length == 0 || phone.replace(/(^s*)|(s*$)/g, "").length == 0 || position.replace(/(^s*)|(s*$)/g, "").length == 0) {
        var onlyChoseAlert = simpleAlert({
            "content": "修改信息不能为空",
            "buttons": {
                "确定": function() {
            ...
            switch (ret["ret"]) {
                ...
                <div class="mockup-content"\
    				...
    				<div\
    					...
    					<a><button class="button" style="background: white" id="showSearchSchedule"\
    						onclick="showSeachTeacherSchedule()">显示日程</button></a>\
    				</div>\
    				<div\
					class="morph-button morph-button-modal morph-button-modal-2 morph-button-fixed">\
					<a href="' + ret["searchTeacherBaiKe"] + '"><button class="button" style="background: white" id="showSearchSchedule"\
						onclick="showSeachTeacherSchedule()">更多信息</button></a>\
				</div>\
                 </div>\
                 <div class="box">\
 				<div class="box-head">\
...
function showSeachTeacherSchedule(){
	var btn = document.getElementById("showSearchSchedule");
	$.ajax({
        url: "/xaingmu/showSchedule",
        async: false,
        type: "POST",
